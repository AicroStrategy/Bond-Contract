...
contract Bond is ERC20, Owned {
    ...
    function mintBonds(uint256 amountUsdc) external {
        ...
        // Mint ABOND tokens to sender
        _mint(msg.sender, amountUsdc);
    ...
    function redeemBonds(uint256 amountBonds) external {
        ...
        if (sqrtExpiryPriceX96 <= sqrtStrikePriceX96) {
            IERC20(USDC).transfer(msg.sender, amountBonds);
        } else {
            amountInAsset = BondPricerLib.getAssetAmountFromBond(amountBonds, sqrtStrikePriceX96);
    ...
    function _getSqrtExpiryPriceX96() internal view returns (uint160) {
